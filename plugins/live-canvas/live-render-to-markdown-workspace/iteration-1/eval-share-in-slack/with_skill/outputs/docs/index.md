# Order Fulfilment Platform

> Exported from the live-render workspace at `<project_root>/.claude/live-render-workspace`
> on `2026-05-14T00:00:00Z`.

## Snapshot

| Type | Count |
| --- | --- |
| Regions | 2 |
| Nodes | 6 |
| Edges | 4 |
| Annotations | 2 |

## Regions

- [Platform services](./platform-services.md) — 3 nodes, 4 edges touching, 1 annotation
- [Data plane](./data-plane.md) — 3 nodes, 3 edges touching, 1 annotation

## World map

```mermaid
flowchart LR
  subgraph platform["Platform services"]
    svc_auth["Auth Service"]
    svc_billing["Billing Service"]
    svc_orders["Orders Service"]
  end
  subgraph data["Data plane"]
    db_users[("User DB")]
    bus_ledger[("Ledger Bus")]
    warehouse_orders[("Orders Warehouse")]
  end

  svc_auth -- "SQL" --> db_users
  svc_billing -- "event" --> bus_ledger
  svc_orders -- "SQL" --> warehouse_orders

  click platform "./platform-services.md" "Open Platform services"
  click data "./data-plane.md" "Open Data plane"

  style svc_auth fill:#0c0d10,stroke:#60a5fa,color:#f0f0f0
  style svc_billing fill:#0c0d10,stroke:#60a5fa,color:#f0f0f0
  style svc_orders fill:#0c0d10,stroke:#60a5fa,color:#f0f0f0
  style db_users fill:#0c0d10,stroke:#34d399,color:#f0f0f0
  style bus_ledger fill:#0c0d10,stroke:#34d399,color:#f0f0f0
  style warehouse_orders fill:#0c0d10,stroke:#34d399,color:#f0f0f0
```

---

_Generated by `live-render-to-markdown` from `<project_root>/.claude/live-render-workspace` at `2026-05-14T00:00:00Z`. To regenerate, re-run the skill against the live-render workspace._
