...
import { addMemory, searchMemories, listMemories, deleteMemory, clearDataset, } from '../../src/tools/memory.js';
...
describe('Memory Tools', () => {
    ...
    describe('addMemory', () => {
        it('should add a memory successfully', async () => {
            ...
            expect(mockDb.addMemory).toHaveBeenCalledWith('user', 'Test content', expect.any(Array), ['test'], undefined);
        });
        ...
        it('should trim whitespace from content and dataset', async () => {
            ...
            mockDb.addMemory.mockResolvedValue(mockMemory);
            await addMemory(mockDb, {
                dataset: '  user  ',
                content: '  Test content  ',
            });
            expect(mockDb.addMemory).toHaveBeenCalledWith('user', 'Test content', expect.any(Array), [], undefined);
        });
    ...
    describe('searchMemories', () => {
        ...
        it('should throw error with empty query', async () => {
            await expect(searchMemories(mockDb, {
                query: '',
            })).rejects.toThrow('Query cannot be empty');
        });
        it('should use default top_k value', async () => {
            mockDb.searchMemories.mockResolvedValue([]);
            await searchMemories(mockDb, {
                query: 'test',
            });
            expect(mockDb.searchMemories).toHaveBeenCalledWith(expect.any(Array), undefined, 5);
        });
    });
    describe('listMemories', () => {
        ...
        it('should use default pagination values', async () => {
            ...
            await listMemories(mockDb, {});
            expect(mockDb.listMemories).toHaveBeenCalledWith(undefined, undefined, 50, 0);
        });
    });
